...
fn main() {
  let output_file_path = Path::new("../src/compiled_scores_turner.rs");
  let mut writer = BufWriter::new(File::create(output_file_path).unwrap());
  let mut buf = "use utils::*;\n\n".to_string();
  let bulge_scores_init: Vec<Score> = [
  ...
  for &(x, y) in &[
    ...
    dangling_scores_3prime[(x.0).0][(x.0).1][x.1] = scale(y);
  }
  buf += &format!(
    "pub const DANGLING_SCORES_3PRIME: DanglingScores = {:?};\n",
    &dangling_scores_3prime
  );
  let mut dangling_scores_5prime = [[[NEG_INF; NUM_BASES]; NUM_BASES]; NUM_BASES];
  for &(x, y) in &[
    ((AU, A), -0.3),
    ((AU, C), -0.3),
    ((AU, G), -0.4),
    ...
    dangling_scores_5prime[(x.0).0][(x.0).1][x.1] = scale(y);
  }
  buf += &format!(
    "pub const DANGLING_SCORES_5PRIME: DanglingScores = {:?};\n",
    &dangling_scores_5prime
  );
  let mut hairpin_scores_init: Vec<Score> = vec![0., 0., 0., 5.4, 5.6, 5.7, 5.4, 6.0, 5.5, 6.4]
    .iter()
    .map(|&x| scale(x))
    .collect();
  ...
  buf += &format!(
    "pub const HAIRPIN_SCORES_INIT: HairpinScoresInit = {:?};\n",
    &hairpin_scores_init
  );
  let interior_scores_init: Vec<Score> = vec![
  ...
  .iter()
  {
    interior_scores_1x1[(x.0).0][(x.0).1][(x.1).0][(x.1).1][(x.2).0][(x.2).1] = scale(y);
  }
  buf += &format!(
    "pub const INTERIOR_SCORES_1X1: InteriorScores1x1 = {:?};\n",
    &interior_scores_1x1
  );
  ...
  .iter()
  {
    interior_scores_1x2[(x.0).0][(x.0).1][((x.1).0).0][((x.1).0).1][(x.1).1][(x.2).0][(x.2).1] =
      scale(y);
  }
  buf += &format!(
    "pub const INTERIOR_SCORES_1X2: InteriorScores1x2 = {:?};\n",
    &interior_scores_1x2
  );
  ...
  .iter()
  {
    interior_scores_2x2[(x.0).0][(x.0).1][((x.1).0).0][((x.1).0).1][((x.1).1).0][((x.1).1).1]
      [(x.2).0][(x.2).1] = scale(y);
  }
  buf += &format!(
    "pub const INTERIOR_SCORES_2X2: InteriorScores2x2 = {:?};\n",
    &interior_scores_2x2
  );

  let mut stack_scores = [[[[NEG_INF; NUM_BASES]; NUM_BASES]; NUM_BASES]; NUM_BASES];
  for &(x, y) in [
    ((AU, AU), -0.9),
    ((AU, CG), -2.2),
    ((AU, GC), -2.1),
  ...
  .iter()
  {
    stack_scores[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
  ...
  .iter()
  {
    terminal_mismatch_scores_hairpin[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
    "pub const TERMINAL_MISMATCH_SCORES_HAIRPIN: TerminalMismatchScores = {:?};\n",
  ...
  .iter()
  {
    terminal_mismatch_scores_interior[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
    "pub const TERMINAL_MISMATCH_SCORES_INTERIOR: TerminalMismatchScores = {:?};\n",
    &terminal_mismatch_scores_interior
  );

  let mut terminal_mismatch_scores_1xmany =
  ...
  .iter()
  {
    terminal_mismatch_scores_1xmany[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
    "pub const TERMINAL_MISMATCH_SCORES_1XMANY: TerminalMismatchScores = {:?};\n",
    &terminal_mismatch_scores_1xmany
  );

  let mut terminal_mismatch_scores_2x3 =
    [[[[NEG_INF; NUM_BASES]; NUM_BASES]; NUM_BASES]; NUM_BASES];
  for &(x, y) in [
    ((AU, AA), 0.7),
    ((AU, AC), 0.7),
  ...
  .iter()
  {
    terminal_mismatch_scores_2x3[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
    "pub const TERMINAL_MISMATCH_SCORES_2X3: TerminalMismatchScores = {:?};\n",
  ...
  .iter()
  {
    terminal_mismatch_scores_multibranch[(x.0).0][(x.0).1][(x.1).0][(x.1).1] = scale(y);
  }
  buf += &format!(
    "pub const TERMINAL_MISMATCH_SCORES_MULTIBRANCH: TerminalMismatchScores = {:?};\n",
